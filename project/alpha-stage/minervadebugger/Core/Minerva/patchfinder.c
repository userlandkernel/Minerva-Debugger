...
static int DecodeBitMasks(unsigned immN, unsigned imms, unsigned immr, int immediate, uint64_t *newval)
{
    ...
    levels = (uint)ZeroExtendOnes(len, 6);
...
__attribute__((unused)) static addr_t
...
__attribute__((unused)) static addr_t kernel_delta = 0;
...
/* these operate on VA ******************************************************/
#define INSN_MSR_TTBR0_EL1_x0 0xD5182000
#define INSN_MSR_TTBR1_EL1_x0 0xD5182020
...
addr_t find_ttbr1_el1_write(void)
{
    addr_t off;
    uint32_t *k;
    k = (uint32_t *)(kernel + xnucore_base);
    for (off = 0; off < xnucore_size - 4; off += 4, k++) {
        if (k[0] == INSN_MSR_TTBR1_EL1_x0) {
            return off + xnucore_base + kerndumpbase;
        }
    }
    k = (uint32_t *)(kernel + prelink_base);
...
addr_t find_ttbr0_el1_write(void)
{
