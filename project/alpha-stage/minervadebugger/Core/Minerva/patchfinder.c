...
find_trustcache(void)
{
    ...
    addr_t __unused cbz, call, func, val, adrp;
    int reg;
    uint32_t op;
...
addr_t
pf_find_symbol(const char *symbol)
{
    if (!symbol) {
        return 0;
    }
...
addr_t find_tcr_el1_write(void){
    addr_t off;
    uint32_t* k;
    k = (uint32_t*)(kernel + xnucore_base);
    for (off = 0; off < xnucore_size - 4; off += 4, k++) {
...
addr_t actual_offset = pf_find_symbol("_" #name); \
if (actual_offset == 0) { \
FIND(name); \
} else { \
addr_t patchfinder_offset = find_ ##name (); \
